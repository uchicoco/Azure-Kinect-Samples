...
void SaveMultipleBodiesToCSV(const std::vector<k4abt_body_t>& bodies, std::ofstream& csvFile, uint64_t timestamp)
{
    try
    {
        ...
        if (csvFile.tellp() == 0)
        {
            ...
            headerStream << ",ANGLE_RIGHT,ANGLE_LEFT,ANGLE_LEGS" << std::endl;
        ...
        for (const auto& body : bodies)
        {
            ...
            double left_arm_angle = CalculateProjectedAngle(jointPositionPelvis, jointPositionNeck, jointPositionNose, jointPositionPelvis, jointPositionShoulderLeft, jointPositionElbowLeft);
